...
Answer every `ASK_FIRST=` line with the user before composing, then pass `--acknowledge`. For the tempo question, record the answer with `--tempo-level <bpm>` instead: it accepts only a value the measurement itself reported, which is the primary from the `tempo` axis or a member of the `tempo_family` axis beside it, refuses anything else while naming every selectable value, and answers that question on its own so `--acknowledge` is not also needed for it. A sheet written before `tempo_family` existed offers only the primary and says so. The chosen level then becomes the BPM the prompt may state and the one `numbers_trace` will accept. An `INFER` tempo carrying a competing metrical level is the most expensive thing on this list to get wrong, and the script will not choose a metrical level for you.
